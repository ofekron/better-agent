...
describe("classifyFlushError", () => {
  ...
  it("terminalizes only the backend's authoritative gone response", () => {
    ...
    expect(classifyFlushError(new HttpStatusError(400, "bad shape"))).toBe("actionable");
    expect(classifyFlushError(new HttpStatusError(409, "conflict"))).toBe("actionable");
  });
});
...
describe("outcomeForCreateError", () => {
  it("stops the drain on a transient error so action order is preserved", () => {
    ...
    expect(outcome.hold).toBeUndefined();
  });
  ...
  it("does NOT stop the drain on a permanent error, and records the dead session", () => {
    ...
    expect(outcome.hold).toEqual({ sessionId: "sess-1", kind: "terminal" });
  });
});
  ...
  it("a permanent create failure does not strand unrelated queued work", () => {
    ...
    for (const entry of queue) {
      ...
      if (entry.type === "create_session") {
        ...
        const err = entry.session.id === "A" ? new HttpStatusError(400, "action required") : null;
        if (err) {
          ...
          if (outcome.hold) failed.add(outcome.hold.sessionId);
          continue;
        }
