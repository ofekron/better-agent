...
export function renderTurnPlan(turn: Turn, mode: RenderMode): RenderToken[] {
  ...
  if (mode === 'collapsed') {
    if (turn.body.length > 0) output.push({ kind: 'ellipsis', id: turn.id })
    appendResult(output, turn)
    return output
  }
  const tasks: Task[] = []
  if (mode === 'live') pushLive(tasks, turn.body)
  else pushBody(tasks, turn.body, 'collapsed')
  drain(tasks, output)
...
export function explanationPlan(explanation: Explanation, expanded: boolean): RenderToken[] {
  return [{ kind: 'explanation', text: explanation.text, itemCount: explanation.itemIds.length, expanded }]
}
...
function drain(tasks: Task[], output: RenderToken[]): void {
  while (tasks.length > 0) {
    ...
    if (item.type === 'Explanation') {
      if (mode === 'live' || mode === 'live-compact') for (const id of item.itemIds) output.push({ kind: 'compact', id })
      else output.push(...explanationPlan(item, mode === 'extended'))
      continue
    }
