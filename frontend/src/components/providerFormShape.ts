...
// Auth modes a kind supports. openai (BA-owned agent loop) is api_key only.
// pi/cursor/kimi/opencode self-auth through their CLI's own login/env keys —
// no BA-managed api_key record, so subscription only. The rest offer both.
