...
- **The `live` flag inside `apply_event` gates exactly three
  side-effects:** the `file_ref_resolver.rewrite_event_data` call,
  the `event_ingester.ingest` write to `events.jsonl`, and the
  `_fire_user_msg_received_if_pending` lifecycle emit. The
  render-tree mutation (dedup + append/replace on `msg.events`) is
