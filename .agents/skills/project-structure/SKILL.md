...
- `backend/bff_server.py` + `backend/bff_app_routes.py`: browser-facing Better Agent application boundary. App-only drafts, UI-selection state, and presentation preferences are owned here; execution preferences use the typed `bff_runtime_service.py` contract authenticated by a dedicated BFF-service-only token with no runtime read/write/control scopes. Unmatched runtime operations are forwarded to the internal runtime endpoint.
