...
Use `query_provider_native_transcript_index` when you need provider-native transcript history. Query the `native_element_fts` table by `sid`, `path`, `cwd`, `element_kind`, `ts`, and `text MATCH ...`; add `LIMIT` in SQL when you want a bounded projection.
