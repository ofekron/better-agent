...
Use `query_provider_native_transcript_index` when you need provider-native transcript history. Query `native_element_fts` for text-only `MATCH` searches. Query `native_element_meta` for metadata filters or recency ordering by `sid`, `path`, `cwd`, `element_kind`, `role`, and `ts_utc`, then join back to `native_element_fts` by `rowid` when you need text. Do not put metadata filters such as `cwd`, `path`, `role`, or `sid` directly on `native_element_fts` together with `MATCH`; use `native_element_meta` for those filters. Use `ts_utc` for chronological ordering and add `LIMIT` in SQL when you want a bounded projection.
