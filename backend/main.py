...
def _harness_profile_selection(body: dict) -> str:
...
async def _admit_offline_batch_message(action: dict) -> dict:
    ...
    if not session:
        raise HTTPException(status_code=404, detail=t("error.session_not_found_retry"))
    offline_error = await _node_offline_error(session)
    if offline_error:
    ...
    is_queued = (
        ...
        or coordinator.has_queued_prompts(session_id)
    )
    queued_prompt = {
        "id": queued_id,
        "lifecycle_msg_id": lifecycle_msg_id,
...
@app.post("/api/offline-actions/batch")
