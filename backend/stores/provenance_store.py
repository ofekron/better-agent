...
Thread-safety: this module holds no lock of its own. In production the
sole writer is ``provenance_ledger_worker``, a single dedicated
background thread — since only one thread ever calls ``record``, there
is nothing to serialize against. Callers that write from more than one
thread for the same ``app_session_id`` (concurrently) must serialize
themselves; direct callers in tests do so sequentially.
"""
...
def _hydrate_seen(app_session_id: str) -> set:
    ...
    from disk on first access (so dedup survives a backend restart)."""
