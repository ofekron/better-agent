...
# Kinds that can change one of the four per-project counters
# (`projects_api.empty_aggregate`: running, unread, waiting_for_user,
# errored) or a session's membership/visibility within its project's
# count (created/deleted/forked/archived_set/selectors_set — the last
# because `set_selectors` is the only mutator of a session's `cwd`, and
# `_project_aggregates` buckets by `(cwd, node_id)`; moving a session
# between projects must invalidate both the old and new bucket).
# Deliberately narrow — a bare `session.*` subscription would invalidate
# the aggregate cache on every chatty per-message event (streaming
# deltas, draft updates, panel state, ...) and defeat the point of
# caching during active sessions.
_PROJECT_AGGREGATE_INVALIDATING_KINDS = frozenset({
    "running_changed",
    "monitoring_changed",
    "unread_changed",
    "seen_advanced",
...
def bind_project_aggregates_invalidation() -> None:
    """Subscribe project-badge cache invalidation to `session.*` BusEvents.

    `projects_api._project_aggregates` caches its per-project running/
    unread/waiting_for_user/errored counters until
    `invalidate_project_aggregates` bumps the generation counter. That
    invalidation previously fired ONLY from project CRUD/auto-add
    mutations (`main.py:_broadcast_projects_changed`) — no session-level
    mutation (a turn starting/stopping, a message going unread/seen, an
    error being set/cleared, a NEEDS_USER_DECISION marker) ever
    invalidated it, so the badges froze stale for the life of the
    process the first time `/api/projects` was read. Subscribing here
    means every future session mutation that can move one of these
    counters invalidates it automatically, without a new call site
    having to remember to do so.

    Idempotent. Priority 50 (default), same tier as the WS broadcaster —
    ordering between the two doesn't matter, both are pure fan-out from
    the same enriched change dict."""
    import projects_api

    async def _handler(event: BusEvent) -> None:
        kind = event.type.removeprefix("session.")
        if kind not in _PROJECT_AGGREGATE_INVALIDATING_KINDS:
            return
        try:
            projects_api.invalidate_project_aggregates()
        except Exception:
            logger.exception(
                "bind_project_aggregates_invalidation: invalidate raised "
                "for %s", event.type,
            )

    bus.unsubscribe("project_aggregates_invalidation")
    bus.subscribe(
        "session.*",
        _handler,
        priority=50,
        name="project_aggregates_invalidation",
    )
    logger.info(
        "event_bus: registered project_aggregates_invalidation "
        "as bus subscriber on session.*",
    )


def unbind_project_aggregates_invalidation() -> None:
    bus.unsubscribe("project_aggregates_invalidation")
