...
def test_cli_prompt_wrapping() -> None:
    ...
    check(agy.startswith("The following injected context is from Better Agent"), "AGY prompt receives capability prefix")
    check("## User prompt\n\nShip it." in agy, "AGY marks the real user prompt")
    check("Ship it." in agy, "AGY prompt preserves user prompt")
...
def test_cli_prompt_wrapping_labels_team_messages_as_messages() -> None:
    ...
    agy = agy_prompt("<mssg>worker result</mssg>", inputs)
    check("## Message\n\n<mssg>" in codex, "Codex labels team messages as Message")
    check("## User prompt\n\n<mssg>" not in codex, "Codex does not label team messages as User prompt")
    check("## Message\n\n<mssg>" in agy, "AGY labels team messages as Message")
    check("## User prompt\n\n<mssg>" not in agy, "AGY does not label team messages as User prompt")
