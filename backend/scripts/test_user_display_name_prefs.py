...
def test_login_username_is_default_display_name() -> None:
    assert app_user_prefs.get_all("ofek")["user_display_name"] == "ofek"


def test_custom_display_name_overrides_login_username() -> None:
    assert app_user_prefs.patch({"user_display_name": "  Ofek   Ron  "})["user_display_name"] == "Ofek Ron"
    assert app_user_prefs.get_all("ofek")["user_display_name"] == "Ofek Ron"


def test_blank_display_name_returns_to_login_username() -> None:
    app_user_prefs.patch({"user_display_name": "Ofek Ron"})
    assert app_user_prefs.patch({"user_display_name": "   "})["user_display_name"] is None
    assert app_user_prefs.get_all("ofek")["user_display_name"] == "ofek"


def test_invalid_display_name_rejected() -> None:
    for value in (True, 7, "x" * (app_user_prefs.MAX_USER_DISPLAY_NAME_LENGTH + 1)):
        try:
            app_user_prefs.patch({"user_display_name": value})
        except ValueError:
            continue
