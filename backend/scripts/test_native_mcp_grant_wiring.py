...
_test_home.isolate("bc-test-native-mcp-grant-wiring-")
...
def test_permission_declaration_round_trips_through_manifest_validation():
    _record()
    record = extension_store.get_extension(EXT_ID)
    assert record is not None
    assert record["manifest"]["permissions"]["native_mcp"] == {SERVER_ID: ["global", "project"]}
    _cleanup()


def test_symlink_escaping_install_root_fails_closed():
    ...
    assert fp is None, f"a symlink escaping install_root must fail the fingerprint closed (got {fp!r})"
    _cleanup()


def test_grant_global_makes_server_resolve_in_native_delivery_configs():
    _record()
    ...
    assert SERVER_ID in configs, f"global grant must surface the server (got {list(configs)})"
    _cleanup()


def test_no_grant_omits_server_from_native_delivery_configs():
    _record()
    configs = extension_store.native_mcp_server_configs({}, user_facing=False, bare=False)
    assert SERVER_ID not in configs, f"no grant must omit the server (got {list(configs)})"
    _cleanup()


def test_bytecode_cache_churn_does_not_false_invalidate_grant():
    ...
    assert SERVER_ID in before
    assert SERVER_ID in after, f"bytecode churn must not false-invalidate (before={list(before)}, after={list(after)})"
    _cleanup()


def test_venv_dependency_change_does_invalidate_grant():
    ...
    after = extension_store.native_mcp_server_configs({}, user_facing=False, bare=False)
    assert SERVER_ID in before
    assert SERVER_ID not in after, f"a .venv change must invalidate the grant (before={list(before)}, after={list(after)})"
    _cleanup()


def test_standalone_pyc_without_py_sibling_is_hashed():
    ...
    after = extension_store.native_mcp_server_configs({}, user_facing=False, bare=False)
    assert SERVER_ID in before
    assert SERVER_ID not in after, f"standalone .pyc content change must invalidate (before={list(before)}, after={list(after)})"
    _cleanup()


def test_grant_gate_admits_identical_server_set_across_provider_wrappers():
    ...
    # the grant check. Proves the actual cross-provider parity claim.
    _record()
    extension_store.grant_native_mcp_server(EXT_ID, SERVER_ID, "global")
    ...
    assert SERVER_ID in claude_configs
    assert SERVER_ID in codex_gemini_configs
    _cleanup()


def test_grant_rejects_ambient_unsafe_server_even_when_declared():
    ...
    # ambiently, even if someone tried. Declaring it in permissions.native_mcp
    # does not make it eligible; eligibility is a property of the entrypoint.
...
def test_project_grant_only_resolves_for_its_own_project():
    _record()
    ...
    key = f"{EXT_ID}:{SERVER_ID}"
    assert key in in_project
    assert key not in other_project
    assert key not in no_project
    _cleanup()


def test_project_grant_does_not_inherit_into_subdirectory():
    ...
    key = f"{EXT_ID}:{SERVER_ID}"
    assert key in in_root
    assert key not in in_subdir
    _cleanup()


def test_manifest_update_invalidates_outstanding_grant():
    ...
    after = extension_store.native_mcp_server_configs({}, user_facing=False, bare=False)
    assert SERVER_ID in before
    assert SERVER_ID not in after, f"stale grant must stop resolving (before={list(before)}, after={list(after)})"
    _cleanup()


def test_server_file_content_change_invalidates_grant_with_identical_manifest():
    ...
    after = extension_store.native_mcp_server_configs({}, user_facing=False, bare=False)
    assert SERVER_ID in before
    assert SERVER_ID not in after, f"server.py content change must invalidate (before={list(before)}, after={list(after)})"
    _cleanup()


def test_grant_survives_no_mirror_step_and_resolves_immediately():
    ...
    # (resolve_native_mcp_servers_for_context), the only remaining authority.
    _record()
    extension_store.grant_native_mcp_server(EXT_ID, SERVER_ID, "global")
    assert len(native_mcp_grants.list_grants(extension_id=EXT_ID)) == 1
    resolved = extension_store.resolve_native_mcp_servers_for_context()
    assert f"{EXT_ID}:{SERVER_ID}" in resolved
    _cleanup()


def test_uninstall_removes_grants():
    _record()
    extension_store.grant_native_mcp_server(EXT_ID, SERVER_ID, "global")
    assert len(native_mcp_grants.list_grants(extension_id=EXT_ID)) == 1
    extension_store.uninstall(EXT_ID)
    assert native_mcp_grants.list_grants(extension_id=EXT_ID) == []
    native_mcp_grants.remove_grants_for_extension(EXT_ID)
...
def test_disabled_extension_grant_resolves_to_nothing():
    ...
    configs = extension_store.native_mcp_server_configs({}, user_facing=False, bare=False)
    assert SERVER_ID not in configs, f"disabled extension must resolve to nothing (got {list(configs)})"
    _cleanup()


def test_config_disabled_builtin_extension_grant_resolves_to_nothing():
    ...
    assert f"{EXT_ID}:{SERVER_ID}" not in resolved, f"config-disabled builtin must resolve to nothing (got {list(resolved)})"
    _cleanup()


def test_revoke_raises_on_unresolvable_project_path_instead_of_silently_coercing():
    ...
    # earlier) -- a NUL byte makes Path.resolve() raise, giving None back
    # from project_target with a truthy project_path.
    _record()
    extension_store.grant_native_mcp_server(EXT_ID, SERVER_ID, "project", project_path="/tmp/proj-a")
    with pytest.raises(extension_store.ExtensionError):
    ...
    assert still_granted, "the real grant must be untouched by the failed revoke"
    _cleanup()


def test_grant_rejects_undeclared_scope():
...
def test_grant_rejects_undeclared_server_id():
    _record()
    with pytest.raises(extension_store.ExtensionError):
...
def test_revoke_removes_only_the_targeted_grant_leaving_siblings_intact():
    package = _record()
    ...
    assert removed
    assert {gr.server_id for gr in remaining} == {other_server}
    _cleanup()


def test_grant_rejects_session_and_turn_scope_in_pr1():
...
def test_set_native_harness_exposed_routes_mcp_kind_through_grants():
    _record()
...
def test_set_native_harness_exposed_mcp_fails_closed_for_undeclared_server():
...
def test_extension_harness_additions_reports_grant_state():
    _record()
    record = extension_store.get_extension(EXT_ID)
    ...
    assert before_entry["native_exposed"] is False
    assert after_entry["native_exposed"] is True
    _cleanup()


def test_resolve_native_mcp_server_config_matches_grant_state():
    _record()
    record = extension_store.get_extension(EXT_ID)
