"""Regression test: a Gemini run where the CLI exits rc=0 without
emitting a `result` event MUST surface a non-null error all the way to
the persisted assistant message and the WS broadcast — never a blank
streaming bubble with no indicator.

Reproduces the incident pattern from `~/.better-claude/runs/6b21133a-…/`
(model `glm-5.1` passed to Gemini provider) and the other CLASS 1/3
silent-failure runs: `gemini -p - -o stream-json -m <bad>` prints an
init line then exits rc=0 with empty stderr → runner_gemini wrote
`complete.json {"success": false, "error": null}` because `error` was
never set (only the `result` branch fills it) → orchestrator's
`set_assistant_error` gate (truthy run_error OR content matches
"API Error: 4xx") never fired → assistant bubble stayed blank with no
error indicator, forever.

We force the exact failure by putting a fake `gemini` stub on PATH that
emits an `init` line and exits rc=0. `runner_gemini._resolve_gemini_cli`
is bare `shutil.which("gemini")` so PATH stubbing works.

Asserts (each guards a distinct layer; one regressing without the
others is a possible regression mode):
  1. complete.json["error"] is non-null (runner contract).
  2. The persisted assistant message has error=True + non-empty
     errorText (orchestrator gate).
  3. A `messages_delta` frame whose messages[0].error is truthy was
     broadcast (frontend wakes up).

Pre-fix code fails (1) because `runner_gemini.error` defaults None and
the rc==0 path never overwrites it. Post-fix code fills error with
"Gemini CLI exited without emitting a result event…" so (1)-(3) all
pass.

Run with:
    cd backend && .venv/bin/python scripts/test_gemini_silent_failure_surfaces_error.py
"""
...
# backend module.
import _test_home
_TMP_HOME = _test_home.isolate("bc-test-gemini-silent-")
...
# Put the stub `gemini` on PATH BEFORE the backend can resolve it.
_STUB_DIR = tempfile.mkdtemp(prefix="bc-test-gemini-stub-")
_STUB_PATH = Path(_STUB_DIR) / "gemini"
# The stub:
#   - reads stdin (runner writes the prompt + closes stdin; if we
#     don't drain, the runner's `proc.stdin.drain()` could block on a
#     small pipe buffer, deadlocking the test before EOF)
#   - writes ONE init line on stdout so bootstrap finds state.json and
#     starts the tailer
#   - exits 0
# It intentionally does NOT emit a `result` event — this is the
# silent-failure shape we lock.
...
def _mint_session_cookie() -> str:
    ...
    so the /ws/chat policy gate accepts our WS connect. Read-only on
    the keychain — never writes credentials."""
...
# The stub gemini accepts any model; we still pass one that exists in
# GEMINI_MODELS so the provider's pre-spawn validation succeeds.
...
async def main() -> int:
    ...
    cwd = tempfile.mkdtemp(prefix="bc-gemsilent-cwd-")
    cookie = _mint_session_cookie()
    cookie_header = {"Cookie": f"bc_session={cookie}"}
    failures = 0
    try:
        # Register + activate a gemini provider so the session can
        # bind to it. The orchestrator looks up the provider by id from
        # the session record.
        prov = config_store.add_provider({
            "name": "gemini-silent-test",
            "kind": "gemini",
            "mode": "subscription",
            "default_model": MODEL,
        })
        ...
        # (1) Runner contract — complete.json["error"] is non-null.
        # Scan the runs dir for the most-recent run owned by this
        # session and read its complete.json.
        ...
        if target is None:
            ...
            if not complete_path.exists():
                ...
                try:
                    ...
                    cdata = {}
                    failures += 1
                if cdata.get("success") is False and cdata.get("error"):
                    print(f"{PASS}  complete.json carries non-null error: "
                          f"{str(cdata['error'])[:80]!r}")
                else:
                    print(f"{FAIL}  complete.json silent-failure shape: "
                          f"success={cdata.get('success')!r} "
                          f"error={cdata.get('error')!r}")
                    failures += 1

        # (2) Orchestrator gate — finalized assistant message has the
