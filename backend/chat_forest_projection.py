...
class ChatForestProjector:
    def project(self, root_id: str, facts: Iterable[CommittedFact]) -> ChatForest:
        ...
        for row in selected:
            ...
            elif fact.payload_type in {"worker_start", "worker_event", "worker_complete", "todos_snapshot"}:
                tree.work.append(WorkNode(
                    id=stable_node_id(fact.payload_type, fact.source_stream_id, fact.source_event_id),
        ...
        return ChatForest(root_id=root_id, canonical_through_seq=through, trees=[prompts[key] for key in order])
